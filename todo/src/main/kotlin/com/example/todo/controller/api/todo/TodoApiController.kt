...
class TodoApiController(
    ...
    @GetMapping(path = [""])
    fun read(@ApiParam(name = "index") @RequestParam(required = false) index:Int?): ResponseEntity<Any?> {  //required = false -> 필수값 아님
       ...
       return index?.let {
        ...
        }?: kotlin.run {  //index 가 없으면 전체조회하는 "/api/todo/all" 여기로 보내라
    ...
    @GetMapping(path = ["/all"])
    fun readAll(): MutableList<TodoDto> {  //required = false -> 필수값 아님
